...
# par(mfrow=c(2,1))
# 
# my.trend <- function(t){coefficients(mod.H2)[1] + coefficients(mod.H2)[2] * t}
# 
# curve(my.trend, from=0, to=length(mod.H2),
#       main="Time series decomposition",
#       xlab="Time",
#       ylab="Trend")
# 
# my.season <- function(t){coefficients(mod.H2)[3]*sin(2*pi*t) + coefficients(mod.H2)[4] * cos(2*pi*t)}
# 
...
<h5> 3. TOČKA: Narišite graf rezidualov in komentirajte, ali so videti stacionarni. Stacionarnost tudi preizkusite z uporabo ustreznih statističnih metod. </h5>
...
<h5> 4. TOČKA: Na rezidualih naredite grafikona ACF in PACF in na njuni podlagi predlagajte vsaj en model vrste AR(p) ali MA(q). </h5>
...
<h5> 5. TOČKA: Na podlagi Yule–Walkerjevih cenilk in kriterija AIC izberite najboljši model AR(p). Primerjajte ga z najboljšim modelom ARMA(p, q) za p + q ≤ 3 po kriteriju AIC (pozor: kriterij AIC je lahko definiran drugače od postopka do postopka). Če je videti smiselno, pa namesto tega uporabite model GARCH. </h5>
...
<h5> 6. TOČKA: Izberite »optimalni« model in ocenite vse njegove parametre. </h5>
...
#linearni trend? ne dela to, sej nevem a to rabiva?
# t <- time(x)
# lint <- lm(x ~ t)
# # n <- as.numeric(lint$coefficients[1])
# # k <- as.numeric(lint$coefficients[2])
# line(time,42.5988+0.2905*time, col = 'red')
...
for(i in 1:3){
    for(j in 1:3){
        my.arma <- stats::arima(d,order=c(i-1,0,j-1),include.mean=FALSE,method="ML")
...
komentar <- paste('Tudi algoritem vrne model MA(1) ali ARMA(0,1).')
print(komentar)

my.arma <- stats::arima(d,order=c(0,0,1),include.mean=FALSE,method="ML")
...
A <- predict(my.arma,n.ahead=10, level = 0.90)$pred

# Harmonic regression predictions

B <- predict(mod.H2,n.ahead=10, level = 0.90)[length(mod.H2)-length(A):length(mod.H2)]
...
#Brina to še ne štima (rima). Moram ugotovit kaj je njuna spremenljivka napoved.
...
Brina, tuki že takoj ko logaritmiraš vidiš, da mava problem. Tisti ostri padci niso okej. Nevem kaj naj s tem nardiva :(
...
Acf(d.res, main = "Acf residuals") #MA(5) ali MA(7), to se mi zdi čudno
Pacf(d.res, main = "Pacf residuals") #AR(5)? ne bi smeli dobit še enga ven pri cca. 12, to je slabo
...
komentar <- paste('Tudi algoritem vrne model MA(1) in AR(1) ali ARMA(1,1).')
print(komentar)
...
```{r, echo=TRUE, eval=TRUE, results="markup",echo=FALSE}
# OPTIMALNI MODEL: izberemo tisti model, ki ima najnižji aic. V najinem primeru je to AR(3). Druga možna izbira bi bila model z MA(1) in AR(2).
komentar <- paste('OPTIMALNI MODEL: izberemo tisti model, ki ima najnižji aic. V najinem primeru je to ARMA(1,1).')
print(komentar)

best <- arima(d.res, order=c(1,0,1))
plot(best$residuals)
hist(best$residuals)
...
komentar <- paste('Shapirov test zavrne hipotezo, torej ne gre za normalno porazdelitev.')
print(komentar)
```
