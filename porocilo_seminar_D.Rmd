...
t <- time(x)
mod.H <- lm(x ~  I(sin(p*t)) + I(cos(p*t)) + I(sin(p2*t)) + I(cos(p2*t)))
summary(mod.H)
...
Box.test(d.res, lag = 1, type="Box-Pierce") #p-value < 2.2e-16 # residuali od best modela
...
B <- predict(mod.H2,n.ahead=10, level = 0.90)[819:828]
...
better_gaussian <- napoved + mean(x.res) + (sd(x.res))/2
