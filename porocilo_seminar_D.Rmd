...
plot(my.trend, from=0, to=length(mod.H2),
      main="Time series decomposition",
      xlab="Time",
      ylab="Trend")

my.season <- function(t){coefficients(mod.H2)[2]*sin(perioda*t) + coefficients(mod.H2)[3] * cos(perioda*t)}
...
#PREDLOG MODELA:
komentar <- paste('Predlagava izbiro modela MA(1) in AR(3), torej ARMA(1,3).')
print(komentar)
...
t <- time(d)
mod.H2 <- lm(d ~  I(sin(perioda*t)) + I(cos(perioda*t)) + I(sin(perioda2*t)) + I(cos(perioda2*t)) +I(sin(perioda3*t)) + I(cos(perioda3*t)) + I(sin(perioda4*t)) + I(cos(perioda4*t)))
summary(mod.H2)
